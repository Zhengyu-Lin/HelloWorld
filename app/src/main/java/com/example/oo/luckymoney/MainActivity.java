...
//        }, 5000L);
    ...
    private void changeLabelStatus() {
        ...
        mAccessibleLabel.setText(isAccessibleEnabled() ? "辅助功能已开启" : "辅助功能未开启");
        mLabelText.setText(isAccessibilityEnabled ? "开关已打开，现在可后台自动抢红包" : "打开开关，设置自动抢红包");

        if (Build.VERSION.SDK_INT >= 18) {
            boolean isNotificationEnabled = isNotificationEnabled();
            mNotificationLabel.setTextColor(isNotificationEnabled ? 0xFF009588 : Color.RED);
            mNotificationLabel.setText(isNotificationEnabled ? "通知使用权已开启" : "通知使用权未开启");

            if (isAccessibilityEnabled && isNotificationEnabled) {
                mLabelText.setText("开关已打开，现在可后台自动抢红包");
            } else {
